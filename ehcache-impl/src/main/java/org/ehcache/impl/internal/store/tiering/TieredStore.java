  ...
  public static class Provider implements Store.Provider {
    ...
    public int rank(final Set<ResourceType<?>> resourceTypes, final Collection<ServiceConfiguration<?, ?>> serviceConfigs) {
      if (resourceTypes.size() == 1) {
        ...
        return trySelect(AuthoritativeTier.Provider.class, serviceProvider, authorityResources, serviceConfigs).map(a -> a.rank(authorityResources, serviceConfigs))
          .flatMap(ar -> trySelect(CachingTier.Provider.class, serviceProvider, cachingResources, serviceConfigs).map(a -> a.rank(cachingResources, serviceConfigs)).map(cr -> ar + cr))
          .orElse(0);
      }
    }
    ...
    private CachingTier.Provider getCachingTierProvider(Set<ResourceType<?>> cachingResources, List<ServiceConfiguration<?, ?>> enhancedServiceConfigs) {
      CachingTier.Provider cachingTierProvider = StoreSupport.select(CachingTier.Provider.class, serviceProvider, cachingResources, enhancedServiceConfigs);
    ...
    AuthoritativeTier.Provider getAuthoritativeTierProvider(Set<ResourceType<?>> authorityResources, List<ServiceConfiguration<?, ?>> enhancedServiceConfigs) {
      AuthoritativeTier.Provider authoritativeTierProvider = StoreSupport.select(AuthoritativeTier.Provider.class, serviceProvider, authorityResources, enhancedServiceConfigs);
