...
public class ChainResolver<K, V> {
  ...
  public Chain compact(Chain chain, long now) {
    Map<K, PutOperation<K, V>> compacted = new HashMap<K, PutOperation<K, V>>(1);
    for (Element element : chain) {
      ByteBuffer payload = element.getPayload();
      Operation<K, V> operation = codec.decode(payload);
      ...
      if(result == null) {
        ...
        if (result instanceof PutOperation<?, ?>) {
          ...
          compacted.put(key, new PutOperation<K, V>(key, result.getValue(), -Long.MAX_VALUE));
        }
      } else {
        if(operation.isExpiryAvailable()) {
          ...
          if (now >= expirationTime) {
            ...
            compacted.put(key, new PutOperation<K, V>(key, result.getValue(), -expirationTime));
          }
        } else {
          try {
            if(previousResult == null) {
              ...
              Duration duration = expiry.getExpiryForUpdate(key, new ValueSupplier<V>() {
                @Override
                public V value() {
                  return previousResult.getValue();
                }
