...
public class LifeCycleMessageActivePassvieReplicationTest {
    ...
    this.clusterControl = PassthroughTestHelpers.createActivePassive(STRIPENAME,
        new PassthroughTestHelpers.ServerInitializer() {
          ...
          public void registerServicesForServer(PassthroughServer server) {
            ...
            server.registerServiceProvider(new OffHeapResourcesProvider(), new OffHeapResourcesConfiguration(resource("test", 32, MemoryUnit.MB)));
  ...
  private static OffheapResourcesType resource(String resourceName, int size, MemoryUnit unit) {
    final OffheapResourcesType resources = new OffheapResourcesType();
