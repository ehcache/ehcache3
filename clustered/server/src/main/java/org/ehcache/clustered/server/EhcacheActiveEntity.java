      ...
      inflightInvalidations.computeIfPresent(message.getCacheId(), (cacheId, tuples) -> {
        ...
        tuples.forEach(tupleOfClientAndSetOfHash -> {
          if (tupleOfClientAndSetOfHash.isClearInProgress()) {
            invalidateAll(tupleOfClientAndSetOfHash.getClientDescriptor(), cacheId);
          }
          tupleOfClientAndSetOfHash.getInvalidationsInProgress()
              .forEach(hashInvalidationToBeResent -> invalidateHashForClient(tupleOfClientAndSetOfHash.getClientDescriptor(), cacheId, hashInvalidationToBeResent));
        });
        return null;
      });
    }
