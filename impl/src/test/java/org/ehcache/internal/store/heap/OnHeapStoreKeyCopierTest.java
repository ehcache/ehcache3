...
public class OnHeapStoreKeyCopierTest {
  ...
  public void setUp() {
    ...
    store = new OnHeapStore<Key, String>(configuration, SystemTimeSource.INSTANCE, keyCopier, new IdentityCopier<String>(), new NoopSizeOfEngine());
  }
  ...
  public void testComputeIfPresent() throws CacheAccessException {
    ...
    store.computeIfPresent(KEY, new BiFunction<Key, String, String>() {
      @Override
      public String apply(Key key, String s) {
        if (copyForRead || copyForWrite) {
          assertThat(key, not(sameInstance(KEY)));
        } else {
          assertThat(key, sameInstance(KEY));
    ...
    if (copyForRead) {
      store.computeIfPresent(KEY, new BiFunction<Key, String, String>() {
        @Override
        public String apply(Key key, String s) {
          assertThat(key, not(sameInstance(keyRef.get())));
          return s;
        }
      });
    }
  ...
  @Test
  public void testComputeIfPresentWithoutReplaceEquals() throws CacheAccessException {
    store.put(KEY, VALUE);
    final AtomicReference<Key> keyRef = new AtomicReference<Key>();
    store.computeIfPresent(KEY, new BiFunction<Key, String, String>() {
      @Override
