...
public class LoaderWriterStoreProvider implements Store.Provider {
  ...
  public int rank(Set<ResourceType<?>> resourceTypes, Collection<ServiceConfiguration<?>> serviceConfigs) {
    ...
    if (resourceTypes.stream().allMatch(x -> x.getTierHeight() != 10)) {
  ...
  private static class StoreRef<K, V> {
    private final Store<K, V> underlyingStore;
    private final Store.Provider underlyingStoreProvider;

    StoreRef(Store<K, V> underlyingStore, Store.Provider underlyingStoreProvider) {
