...
<1> A new feature of Ehcache 3.0 is the ability to create `UserManagedCache` instances, i.e. ones not managed by a `CacheManager`;
<2> As there is nothing that manages them, it is up to you to `UserManaged.init()` them, prior to using them.
