...
link:xsds{outfilesuffix}#_core[this schema definition], namely:

=== `<config>` root element

The root element of our XML configuration. One `<config>` element, and as such one XML file, corresponds to
a `CacheManager`. From Ehcache's perspective though, you are not prohibited to create multiple `CacheManager` instances
of one XML configuration file. Ehcache does _not_ act as a repository of `CacheManager`, unlike JSR-107's
...
`<cache>` elements each represent a `Cache` that will be created, and managed, by the `CacheManager`.
...
NOTE: We've setup a complete configuration link:examples{outfilesuffix}#_xml_with_107_extension[example] for you to inspire from.

== XML programmatic parsing

NOTE: If you are obtaining your `CacheManager` through the JSR-107 API, what follows below is done automatically
...
<2> The inherent properties can be overriden by simply providing a different value prior to building the `CacheConfiguration`
