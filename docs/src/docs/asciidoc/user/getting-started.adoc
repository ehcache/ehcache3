...
<1> You can also size a tiered cache based on bytes only.You can limit the amount of java heap used by cache for storing key-value pairs. 
<2> A SizeOfEngine can also be provided which actually ascertains the size of objects on java heap. It uses sizeof library to do so. The configuration for the same takes two arguments. First one specifies the maximum
number of objects that will be visited while walking the object graph, the other argument defines the maximum limit of the size of the object. All mutations, which cross the limits will be ignored.
