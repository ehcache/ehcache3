...
----
Ehcache 3 adds support for the Jakarta EE namespaced versions of the JTA (for the transactions module) and JAXB (for parsing Ehcache XML files) APIs.
...
The modules ehcache-xml and ehcache have a dependency upon JAXB.  If you are using JDK-9 or greater than you need to ensure the JAXB dependency is available at runtime.

Please note:

The JAXB dependency declared by Ehcache is with the version range 2.2.0 - 3.0.0.  You will probably want to specify a particular version, in order to get consistent builds of your components.


*Jakarta Variants*

If your deployment environment has moved to the new Jakarta EE namespaced APIs then you will want to choose the Jakarta namesapced Ehcache variants.

Here are the Jakarta variant JAXB dependency co-ordinates :
...
In a maven build the POM contents does not capture the dependencies of the Jakarta variant. You will need to manually add a dependency on a suitable JAXB runtime.
Ehcache will work with any Jakarta JAXB runtime in the range:
