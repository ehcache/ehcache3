...
ehcache-xml and ehcache modules requires jaxb dependency at runtime, So If you are using java 1.9+ version then you should make jaxb dependency available at runtime.

Prefer version for jaxb dependency : [2.2,3) means: greater than or equal to 2.2 but less than 3
