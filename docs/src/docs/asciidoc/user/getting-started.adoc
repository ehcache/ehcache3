...
<6> Enable the write coalescing behavior, which ensures that only latest update on a key hits the
    underlying resource.
<7> Control maximum number of updates allowed per second.
<8> Configure maximum and minimum number of seconds to wait before writing to the underlying resource.
