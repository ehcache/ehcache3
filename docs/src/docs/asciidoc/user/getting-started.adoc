...
ehcache-xml and ehcache modules requires jaxb dependency. So If you are using jdk-9+ version then you should make jaxb dependency available at runtime.

1. We depend on a version range, Version for jaxb dependency should range between 2.2.0 - 3.0.0
2. We have upgraded the jaxb api to jakarta version

Here are the jaxb dependency co-ordinates :
