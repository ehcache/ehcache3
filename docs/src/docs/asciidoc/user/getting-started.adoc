...
Serialization is primarily used in ehcache to store the data in the non-heap tiers. So whenever the user uses a
multi-tiered cache, serialization and deserialization happens when the data is moved between the tiers. Ehcache provides
a serializer that can serialize any `Serializable` type. So if you are using standard `Serializable` types as your keys
and values, no extra serializers need to be configured as the inbuilt serializer would handle those cases. But if you
use your own custom types in ehcache that are not serializable, the stock serializer won't be able to perform the
serialization/deserialization on its own. So you need to provide the serialization logic as well. Inorder to do this you
can register your own custom serializers to the cache/cache manager. So if you register a serializer for some type of
data, that serializer would be used to perform all the serialization/deserialization for that type.
...
before storing them in the cache. Ehcache supports store-by-reference by default and the user needs to configure the
cache explicitly to use store-by-value. The user can specify custom `Copiers` for keys and values to be used for
copying. The user can create a copier by implementing the Copier<T> interface directly if he has separate copying logic
for reads and writes or implement the helper class `ReadWriteCopier` that just has a_copy(T obj)_ method that needs to be
overridden with the generic copying logic for both reads and writes for the type used. If no copiers are provided, the
cache will be a store-by-reference cache.
...
If you look at the cache configuration in the above example, there are no copiers configured for the values, and that is
fine in ehcache. If a copier is not configured, we fall back to store-by-reference. So with the above configuration, you
get a cache that stores keys by-value and values by-reference.
...
We are now working on the next milestone, aka https://github.com/ehcache/ehcache3/milestones/Milestone%203[Milestone 3] :
