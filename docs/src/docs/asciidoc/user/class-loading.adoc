...
`public XmlConfiguration(URL url, final ClassLoader classLoader, final Map<String, ClassLoader> cacheClassLoaders)`::
  to use a specific `ClassLoader` at the `CacheManager` level
  and a map of `<String, ClassLoader>` which will be used to link a specific `ClassLoader` to a `Cache` by its alias
...
If no class loaders are specified, the <<cl-default,default class loader>> from Ehcache will be used.
