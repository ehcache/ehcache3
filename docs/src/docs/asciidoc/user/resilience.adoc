...
right away. The result is that the caller will more or less behave as if the cache was disabled.
...
Your disk storage might, rarely, unless you used a network drive. But then, you are running for it.
...
* Connection: When establish connection to the server (default: 150 seconds)

Timeouts can be configured using a dedicated builder or in XML.

[source%nowrap,java,indent=0]
----
include::{sourcedir34}/clustered/client/src/test/java/org/ehcache/clustered/client/docs/Resilience.java[tag=timeoutsExample]
----
...
By default, a lease last 150 seconds.
