...
When a serializer class is configured (either at the cache or cache manager level), since Ehcache is responsible for creating the
instance, it also is responsible for disposing of it. If a serializer class name is configured and that class implements `java.io.Closeable` then
...
When a copier class is configured (either at the cache or cache manager level), since Ehcache is responsible for creating the
instance, it also is responsible for disposing of it. If a copier class name is configured and that class implements `java.io.Closeable` then
`close()` will be called when the cache gets disposed of.
