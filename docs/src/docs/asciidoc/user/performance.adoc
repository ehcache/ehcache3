...
We won't talk about clustering here because it's a different realm and its performance are based on many factors.
...
However, it will be faster for an entry found is indeed found there.
...
When using bytes, we need to calculated the size of every object added to the cache.
...
You might want to use a copier for whatever reason.
...
We are expecting it to be a tiny slower.
