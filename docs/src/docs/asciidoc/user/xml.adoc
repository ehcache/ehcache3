...
Requires serializer `type` and a fully qualified class name of the `Serializer`.   

=== `<default-copiers>` element

A `<default-copiers>` element represents `Copiers` configured at `CacheManager` level.
Requires copier `type` and a fully qualified class name of the `Copier`.

=== `<persistence>` element

A `<persistence>` element represents `Persistence`, to be used in case of persistent `Cache` instances.
Requires the `directory` location where data needs be stored on disk.
