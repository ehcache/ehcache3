...
Distributed Caching can be enabled on a cache by adding Clustered storage as a tier to the cache.
This allows users to harness additional benefits of horizontal scale-out, without loosing on latency offered by local on-heap tiers.
...
<2> Configuring clustered tier of fixed size from server resource.
...
Expiry in clustered caches work with an exception that `Expiry#getExpiryForAccess` is ignored in clustered tier, though the same
is honored in on-heap tiers. Thus no entry gets expired in clustered storage tier based on `getExpiryForAccess`.
