...
Each Ehcache _Clustered Tier Manager_ at the server side is identified by a unique identifier.
...
Dedicated pool::
Dedicated pools are a fixed number of storage pools allocated to the clustered tiers of caches.
...
Shared pool::
Shared pools are also a fixed number of storage pools, but can be shared by the clustered tiers of multiple caches.
