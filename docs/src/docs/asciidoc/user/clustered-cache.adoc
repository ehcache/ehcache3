...
Unknown pool::
An Unknown pool, also referred to as a Clustered pool, allows a client the ability to inherit a pre-configured resource pool from an existing cache.
For example, if there is already a dedicated or shared resource pool configured for cache1 and a client would like to use its resource pool configuration for cache2 it may do so by specifying an Unknown pool in its cache configuration.
Please refer to Unknown Clustered Inheritance example
...
=== Clustered Unknown Inheritance

We have included an option which allows a cache to be created without specifying its clustered resource pool allocation.
In order to do so you must already have a cache created as either Shared or Dedicated.
When you create the second cache(with the same name) in a different cache manager you must configure it with a clustered unknown resource pool.
...
<5> Build a cache configuration for a clustered Unknown resource pool, which will use the previously configured clustered Dedicated resource pool.
<6> Create cache with the same name "my-dedicated-cache" and use the clustered Unknown cache configuration
