...
To enable Clustering with Terracotta, you will have to deploy a http://terracotta.org/downloads/open-source/catalog[Terracotta server] configured with clustered cache storage.  For convenience, the downloadable Ehcache kit contains the Terracotta Server since Ehcache version 3.1.
...
Ehcache Clustered Tier Manager::
_Clustered Tier Manager_ is the server-side entity that gives clustering capabilities to a cache manager.
...
An Ehcache Clustered Tier Manager at the server side is identified by a unique identifier.
Multiple cache managers can identify and connect to the same clustered entity using this unique identifier.

Dedicated pool::
Dedicated pools are a fixed-amount of storage pools allocated to the clustered tiers of caches.
A dedicated amount of storage is allocated directly from server off-heap resources to these pools.
Caches will have exclusive access to the storage resources in the dedicated pool that their clustered tiers are configured with.

Shared pool::
Shared pools are also fixed-amount storage pools, but can be shared by the clustered tiers of multiple caches.
...
If a cache needs a fixed-amount of storage for its clustered tier that it is not willing to share, then it must allocate a dedicated pool.
+
...
Consequently, when the real time clustered tier contents of these caches attempts to exceed the total capacity of their shared resource pool, some cache data may be evicted.
For instance if two caches are already at the shared resource pool size limit, and cache1 attempts to expand, then cache2 may have entries evicted to compensate for the need for more space.

Unspecified pool::
An Unspecified pool, also referred to as a Clustered pool, allows a client the ability to inherit a pre-configured resource pool from an existing cache.
For example, if there is already a dedicated or shared resource pool configured for cache1 and a client would like to use its resource pool configuration for cache2 it may do so by specifying an Unspecified pool in its cache configuration.
Please refer to Unspecified Clustered Inheritance example
...
We have included an option which allows a cache to be created without specifying its clustered resource pool allocation.
In order to do so you must already have a cache created with either a Shared or Dedicated resource pool.
When you create the second cache (with the same alias) in a different cache manager you must configure it with a clustered unspecified resource pool.
This second cache will then inherit the clustered resource pool of the first cache.

This option provides many benefits.
The main benefit is it simplifies clustered configuration by allowing clustered resource pool configuration to be handled by one client, then all subsequent clients can inherit this configuration.
...
[source,java,indent=0]
----
include::{sourcedir31}/clustered/client/src/test/java/org/ehcache/clustered/client/docs/GettingStarted.java[tag=unknownClusteredCacheExample]
----
...
<3> Create cache "my-dedicated-cache" using the Dedicated cache configuration
