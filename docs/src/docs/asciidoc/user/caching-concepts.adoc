...
store, these stores are referred to as tiers.
...
is less hot remains in the slower (and more abundant) tiers.

== Topology Types

=== Standalone

The data set is held in the application node. 
If a standalone topology is used where there are multiple application nodes running the same application, then their caches are completely independent.
