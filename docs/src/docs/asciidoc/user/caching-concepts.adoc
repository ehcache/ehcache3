...
Data "freshness" refers to aspect of a copy of data (e.g. in a cache) being up-to-date with the source version of the
data (e.g. in the syste-of-record (SoR)).  A "stale" copy is considered to be out of sync (or likely to be out of
sync) with the SoR.
...
components that have loaded data from the SoR have a no magic way of ensuring that data is not stale.

==== Cache Entry Expiration

Ehcache can assist you with reducing the likelihood that stale data is used by your application by "expiring" cache
...
time-to-live "TTL" setting.  Or to expire entries 17 seconds after the last time the entry was retrieved from the
cache - which is a time-to-idle "TTI" setting.

The expiration configuration that would be most appropriate for your cache (if any) would be a mixture of a business
and technical decision based upon the requirement assumptions of your application.



=== Storage Tiers
You can configure a cache to use various data storage areas.  When a cache is configured to use more than one data
store, they become referred to as tiers.
...
* Cluster Store - Utilizes out-of-process storage (across network) to store data.  This also enables the sharing of
cached data between all processes utilizing the clustered storage.

When a cache is configured to use more than one storage area, those areas are arranged and managed as tiers - where
the "hotest" (most recently accessed) data is kept in the faster (and typically less abundant) tiers, and data that
is less hot is moved to the slower (and more abundant) tiers.


=== Topology Types

* Standalone – The data set is held in the application node. Any other application nodes are independent with no
...
* Distributed / Clustered – The data is held in a remote server (or array of servers) with a subset of hot
data held in each application node. This topology offers offers a selection of consistency options.
