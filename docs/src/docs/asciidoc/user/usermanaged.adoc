...
<4> In the same vein, a `UserManagedCache` requires you to `UserManagedCache.close()` it explicitly. If you also use
    managed caches simultaneously, the `CacheManager.close()` operation does not impact the user managed cache(s).
...
<2> Pass the persistence service to the builder as well as an id for the cache - note that this will make the builder produce a more specific type: `PersistentUserManagedCache`
