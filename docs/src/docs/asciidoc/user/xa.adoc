...
<2> Configure the cache manager such as it can handle transactions by having a `TransactionManagerProvider` loaded.
...
<1> First start the Bitronix transaction manager. By default, Ehcache 3 will auto-detect it but will throw an exception
    during the cache manager initialization if BTM isn't started.
<2> Specify a `TransactionManagerProviderConfiguration` containing a `TransactionManagerWrapper` instance to be added to
    the builder. The `TransactionManagerWrapper` class is constructed with both the `javax.transaction.TransactionManager`
