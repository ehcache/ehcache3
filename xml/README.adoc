...
Supported nested elements are optional:
 ...
 . `<integration>`: lets you configure `CacheLoader` and `CacheWriter` for a _cache-through_ pattern
...
NOTE: We've setup a complete configuration link:src/test/resources/ehcache-example.xml[example] for you to inspire from.
