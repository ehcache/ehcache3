...
It is the `CacheStore` that wires the authority and caching tiers and provides a unified view of the `Store` for the Cache.
...
- In a two tier `CachingTier`, mappings are present in one tier or the other but never both at the same time. This contract is enforced by having all operations happen atomically from the higher tier's point of view.
...
- All multi-tiered `Cache` have only one store i.e. `CacheStore` which wires the
