...
<4> Add a `XAStoreConfiguration` object to make the cache XA transactional. You must also give the cache a unique
    XAResource identifier as some transaction managers require this.
<5> Add a `CacheLoaderWriter` configuration. This one is a mocked SoR backed by a map for illustration purpose that
...
[source,java,indent=0]
----
include::{sourcedir}/transactions/src/test/java/org/ehcache/transactions/xa/docs/XAGettingStarted.java[tag=testNonTransactionalAccess]
----
...
<4> Add a `XAStoreConfiguration` object to make the cache XA transactional. You must also give the cache a unique
    XAResource identifier as some transaction managers require this.
<5> The cache is being accessed with no prior call to `transactionManager.begin()` which makes it throw `XACacheException`.
